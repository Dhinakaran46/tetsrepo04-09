...
export class BarcodePrintingComponent {
  ...
  onSubmit(isRFID = false) {
    ...
    if (formData.no_of_barcodes > 0) {
      ...
      this.generateBarcodes(currentLabelEnd, noOfBarcodes, formData.prefix, zplCodeTemplate, isRFID);
    } else {
      this.loading = false;
  ...
  generateBarcodes(startLabelEnd: string, noOfBarcodes: number, prefix: string, zplCodeTemplate: string, isRFID: boolean) {
    this.barcodes = [];
    this.zplcodes = [];
    this.loading = true;
    ...
    this.createPDFWithZPLImages(this.zplcodes, newLabelEnd.toString().padStart(labelLength, '0'), isRFID);
  }

  async createPDFWithZPLImages(zplcodes: string[], newLabelEnd: any, isRFID: boolean) {
    const pdfDoc = await PDFDocument.create();
    const dpi = 96; // Dots per inch (DPI) for conversion
    if (isRFID) {
      // Use a standard A4 page (you can also use [612, 792] for Letter) [595, 842]
      ...
      for (let i = 0; i < zplcodes.length; i++) {
        ...
        const lines = zplCode.split(/\r?\n/);

        for (const line of lines) {
          // Hard-wrap each line
          const wrappedLines = this.hardWrap(line, font, fontSize, maxWidth);
  ...
  hardWrap(text: string, font: PDFFont, fontSize: number, maxWidth: number): string[] {
    let lines: string[] = [];
    let current = '';
