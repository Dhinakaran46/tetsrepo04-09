...
export class DateRangePickerComponent implements AfterViewInit, OnChanges {
  ...
  private detectQuickRange(): QuickDateRange | null {
    ...
    if (this.isSameDay(this._pendingDateRange.fromDate, today) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'today';
    }

    if (this.isSameDay(this._pendingDateRange.fromDate, yesterday) && this.isSameDay(this._pendingDateRange.toDate, yesterday)) {
      return 'yesterday';
    }
    ...
    if (this.isSameDay(this._pendingDateRange.fromDate, startOfWeek) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'thisWeek';
    }
    ...
    if (this.isSameDay(this._pendingDateRange.fromDate, lastWeekStart) && this.isSameDay(this._pendingDateRange.toDate, lastWeekEnd)) {
      return 'lastWeek';
    }

    // Check for this month
    const startOfMonth = new Date(today.getFullYear(), today.getMonth(), 1);
    if (this.isSameDay(this._pendingDateRange.fromDate, startOfMonth) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'thisMonth';
    }
    ...
    if (this.isSameDay(this._pendingDateRange.fromDate, lastMonthStart) && this.isSameDay(this._pendingDateRange.toDate, lastMonthEnd)) {
      return 'lastMonth';
    }

    // Check for last 3 months
    const threeMonthsAgo = new Date(today.getFullYear(), today.getMonth() - 2, 1);
    if (this.isSameDay(this._pendingDateRange.fromDate, threeMonthsAgo) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'last3Months';
    }

    // Check for last 6 months
    const sixMonthsAgo = new Date(today.getFullYear(), today.getMonth() - 5, 1);
    if (this.isSameDay(this._pendingDateRange.fromDate, sixMonthsAgo) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'last6Months';
    }

    // Check for this year
    const startOfYear = new Date(today.getFullYear(), 0, 1);
    if (this.isSameDay(this._pendingDateRange.fromDate, startOfYear) && this.isSameDay(this._pendingDateRange.toDate, today)) {
      return 'thisYear';
    }
    ...
    if (this.isSameDay(this._pendingDateRange.fromDate, defaultStart) && this.isSameDay(this._pendingDateRange.toDate, defaultEnd)) {
      return 'defaultDate';
    }
  ...
  private updateInputFields(): void {
    ...
    if (this.toDateInput?.nativeElement) {
      this.toDateInput.nativeElement.value = this._pendingDateRange.toDate ? this.formatDateForInput(this._pendingDateRange.toDate) : '';
